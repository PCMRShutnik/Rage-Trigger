...
function CanCast1( npcEnemy )
	if logic.IsEnemy(npcEnemy)== true
	then
...
function Consider1()
	...
	then
		if(ManaPercentage>0.4 and (npcBot:GetHealth()/npcBot:GetMaxHealth()>0.8 or npcBot:HasModifier("modifier_abaddon_aphotic_shield")) and ability:GetLevel()>=2 )
		then
			if (WeakestEnemy~=nil)
			then
				if ( CanCast[abilityNumber]( WeakestEnemy ) )
