...
local function GetComboDamage()
	...
	then
		sum=sum+AbilitiesReal[3]:GetAbilityDamage()
	end
...
function Consider4()
	...
	then
		...
		if ( #towers>0) 
		then
			return BOT_ACTION_DESIRE_HIGH,logic.GetUnitsTowardsLocation(towers[1],GetAncient(GetTeam()),300)
		end
	end
	...
	then
		...
		then
			return BOT_ACTION_DESIRE_HIGH,logic.GetUnitsTowardsLocation(GetAncient(GetTeam()),towers[1],300)
		end
	end
