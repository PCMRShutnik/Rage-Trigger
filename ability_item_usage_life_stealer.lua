...
local castTarget = {}
local castType = {}
local CanCast={logic.NCanCast,logic.NCanCast,logic.NCanCast,logic.NCanCast,logic.NCanCast,logic.NCanCast,logic.NCanCast,logic.NCanCast}
...
function Consider6()
	...
	local CreepHealth=10000
	local allys = npcBot:GetNearbyHeroes( 1200, false, BOT_MODE_NONE );
	local StrongestAlly,AllyHealth=logic.GetStrongestUnit(allys)
	local enemys = npcBot:GetNearbyHeroes(CastRange+300,true,BOT_MODE_NONE)
	local WeakestEnemy,HeroHealth=logic.GetWeakestUnit(enemys)
	local creeps = npcBot:GetNearbyCreeps(CastRange+300,true)
	local WeakestCreep,CreepHealth=logic.GetWeakestUnit(creeps)
	local StrongestCreep,CreepHealthEnemy=logic.GetStrongestUnit(creeps)
	local creepsAlly = npcBot:GetNearbyCreeps(CastRange+300,false)
	local StrongestCreepAlly,CreepHealthAlly=logic.GetStrongestUnit(creepsAlly)
