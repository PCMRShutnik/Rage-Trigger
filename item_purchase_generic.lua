...
if string.find(GetBot():GetUnitName(), "hero") and not GetBot():IsIllusion() then
    purchase = require(GetScriptDirectory() .. "/Shopping cart/item_build_" .. string.gsub(GetBot():GetUnitName(), "npc_dota_hero_", ""))
end
if purchase == "NOT IMPLEMENTED" then 
	return 
end
local role = require(GetScriptDirectory() .. "/Rolelogic");
local items = require(GetScriptDirectory() .. "/Itemlogic" )
npcBot = GetBot()
...
function PurchaseWard()
	...
	if( GetItemStockCount( "item_ward_observer" ) > 1 and 
...
function PurchaseDust()
	if ( DotaTime() > 2*60 ) 
	then
		if( (npcBot:GetGold() + 250) >= GetItemCost( "item_dust" ) and
